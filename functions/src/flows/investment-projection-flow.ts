...
const InputSchema = z.object({
  ...
  investmentType:    z.string(),  // CDB | LCI | LCA | Tesouro | Fundo | Acao | ...
  institution:       z.string().optional(),
  investedAmount:    z.number(),
  benchmarkType:     z.string(),  // CDI | Selic | IPCA | Prefixado | Personalizado
  benchmarkPercent:  z.number(),  // ex: 100 (100% CDI)
  fixedRateAnnual:   z.number().optional(), // para Prefixado/IPCA+
...
const ProjectionPointSchema = z.object({
  label:      z.string(),   // "Hoje", "3 meses", "6 meses", "Vencimento"
  date:       z.string(),   // YYYY-MM-DD
  grossValue: z.number(),
  netValue:   z.number(),   // após IR (igual ao bruto para isentos)
  earnings:   z.number(),   // rendimento acumulado bruto
  percent:    z.number(),   // % de rendimento bruto
});

const OutputSchema = z.object({
  projectionPoints:       z.array(ProjectionPointSchema),
  annualYieldEffective:   z.number(),
  monthlyYieldEffective:  z.number(),
  irAliquot:              z.number(),    // alíquota IR aplicável (0 se isento)
  irValue:                z.number(),    // valor do IR no vencimento
  netEarningsAtMaturity:  z.number(),
  grossEarningsAtMaturity:z.number(),
  summary:                z.string().max(300),
  riskNote:               z.string().max(200),
  benchmarkComparison:    z.string().max(200),
});

export type InvestmentProjectionOutput = z.infer<typeof OutputSchema>;

export const investmentProjectionFlow = ai.defineFlow(
  {
  ...
  async (input) => {
    const {
      investmentName, investmentType, institution, investedAmount,
      benchmarkType, benchmarkPercent, fixedRateAnnual,
      startDate, maturityDate, liquidity,
      selicAnual, cdiAnual, ipcaMensal, ipcaAnual, todayDate,
    } = input;
...
${isVariableIncome ? `\n⚠️ ${investmentType} é renda variável — não projete rendimento fixo, use o valor investido como referência.` : ''}`;
    ...
    const benchmarkDesc =
      benchmarkType === 'CDI'       ? `${benchmarkPercent}% do CDI (${(cdiAnual * benchmarkPercent / 100).toFixed(2)}% a.a. efetivo)` :
