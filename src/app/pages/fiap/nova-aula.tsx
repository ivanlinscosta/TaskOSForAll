  ...
  useEffect(() => {
    if (aiData) {
      setFormData((prev) => ({
        ...
        planoDeAula: aiData.planoDeAula || prev.planoDeAula,
      }));
    }
  }, [aiData]);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-[var(--theme-muted-foreground)]">
            Cadastre uma aula vinculada a uma disciplina
          </p>
        </div>
      </div>
            ...
            <div className="space-y-3">
              ...
              {formData.topicos.map((item, index) => (
                <div key={index} className="flex gap-2">
                  <Input
                    value={item}
                    onChange={(e) => updateArrayField('topicos', index, e.target.value)}
                  />
